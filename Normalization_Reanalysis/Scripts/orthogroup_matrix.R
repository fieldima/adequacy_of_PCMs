...
#Load Orthogroups
orthogroups_all <- read_tsv(snakemake@input[["ogs"]])
single_copy <- read_lines(snakemake@input[["sc_ogs"]]) %>% as.data.frame() %>% rename(Orthogroup = ".")
...
folders_lvl1 <- list.dirs(Path, full.names = FALSE,recursive = FALSE)
...
write_tsv(tpm_matrix, file = paste0(Path, "TPM_data.tsv"))
write_tsv(fpkm_matrix, file = paste0(Path, "FPKM_data.tsv"))